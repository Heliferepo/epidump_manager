...
dependencies_installer() {
    ...
    local packages=$(curl https://raw.githubusercontent.com/Epitech/dump/master/install_packages_dump.sh | grep -oPz 'packages_list=\(\K[^\)]+' | sed 's/x264.x96_64//g'; sed 's/lightspark.x86_64//g'; sed 's/lightspark-mozilla-plugin.x86_64//g')
