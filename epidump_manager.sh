...
show_help() {
	...
	echo -e "\t-v\tinstall Vim with dnf and vim-epitech (USER Install) (This is not an offcial Vim Epitech Script)"
...
check_for_vim() {
	check_for_file_and_install_package_if_not_present "Vi Improved" /usr/bin/vim vim
}
...
rebuild_all() {
	...
	local lastPWD=${PWD}
	echo "Cloning Epitech's dump repository to /tmp/${epitech_dump_directory}..."
	local epitech_dump_directory=/tmp/epitech-dump
	git clone https://github.com/Epitech/dump ${epitech_dump_directory}

	if [ ! -d "${epitech_dump_directory}" ]; then
		echo "Could not download epitech-dimp... Please report this to the repository owner and check your internet connection"
		exit 1;
	fi
	...
	echo "Executing Epitech install_packages_dump.sh script... (Unsafe)"
	./install_packages_dump.sh
	cd ${lastPWD}
	rm -rf ${epitech_dump_directory}
	reinstall_epitech_vim
}
...
dependencies_installer() {
	...
	if [ -z "$packages" ]; then
		echo "Could not get the package list" 1>&2
		echo "Please report this to the repo manager by creating an issue" 1>&2
		exit 1
	fi
	...
	if [[ $? -ne 0 ]]; then
		echo "Could not install HaskellStack properly"
		exit 1
	fi
}
...
reinstall_epitech_vim() {
	...
	echo "Cloning vim-epitech repo..."
	git clone https://github.com/Heliferepo/vim-epitech
	if [ ! -d "vim-epitech" ]; then
		echo "Could not download vim-epitech... Aborting..."
		exit 1
	fi
	cd vim-epitech
...
# Implements -c
criterion_installer() {
	echo "Adding RPM of criterion samber/criterion-rpm-package"
	rpm -ivh https://github.com/samber/criterion-rpm-package/releases/download/2.3.3/libcriterion-devel-2.3.3-2.el7.x86_64.rpm
	echo "Installing criterion trough Heliferepo/epidump_manager/install_criterion.sh"
...
launch() {
	...
	if [ "$EPIVIM" == 1 ]; then
		echo "Installing / Reinstalling vim-epitech"
		reinstall_epitech_vim
	fi
}
